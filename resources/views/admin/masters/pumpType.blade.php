<x-admin.layout>
    <x-slot name="title">Pump Type</x-slot>
    <x-slot name="heading">Pump Type</x-slot>
                ...
                <div class="card">
                    <form class="theme-form" name="addForm" id="addForm" enctype="multipart/form-data">
                        ...
                        <div class="card-header">
                            <h4 class="card-title">Add Pump Type</h4>
                        </div>
                        <div class="card-body">
                            <div class="mb-3 row">
                                <div class="col-md-4">
        ...
        <div class="row" id="editContainer" style="display:none;">
            <div class="col">
                <form class="form-horizontal form-bordered" method="post" id="editForm">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Edit Pump Type</h4>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="buttons-datatables" class="table table-bordered nowrap align-middle" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($PumpType as $Pump)
                                        <tr>
                                            <td>{{ $loop->iteration}}</td>
                                            <td>{{ $Pump->Description }}</td>
                                            <td>{{ $Pump->Description_regional }}</td>
                                            <td>{{ $Pump->value }}</td>
                                            <td>{{ $Pump->other_value }}</td>
                                            <td>
                                                <button class="edit-element btn text-secondary px-2 py-1" title="Edit PumpType" data-id="{{ $Pump->id }}"><i data-feather="edit"></i></button>
                                                <button class="btn text-danger rem-element px-2 py-1" title="Delete PumpType" data-id="{{ $Pump->id }}"><i data-feather="trash-2"></i> </button>
                                            </td>
                                        </tr>
                                    @endforeach
                            </table>
            ...
            success: function(data)
            {
                ...
                if (!data.error2)
                    swal("Successful!", data.success, "success")
                        .then((action) => {
                            window.location.href = '{{ route('pump-type.index') }}';
                        });
                else
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('pump-type.edit', ":model_id") }}";
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (!data.error)
                {
                    $("#editForm input[id='edit_model_id']").val(data.PumpType.id);
                    $("#editForm input[name='Description']").val(data.PumpType.Description);
                    $("#editForm input[name='Description_regional']").val(data.PumpType.Description_regional);
                    $("#editForm input[name='value']").val(data.PumpType.value);
                    $("#editForm input[name='other_value']").val(data.PumpType.other_value);
                }
                else
                {
                    alert(data.error);
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('pump-type.update', ":model_id") }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
...
<script>
    $("#buttons-datatables").on("click", ".rem-element", function(e) {
        e.preventDefault();
        swal({
            title: "Are you sure to delete this Pump Type?",
        ...
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('pump-type.destroy', ":model_id") }}";
