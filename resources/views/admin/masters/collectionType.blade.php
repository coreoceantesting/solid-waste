<x-admin.layout>
    <x-slot name="title">Collection Type</x-slot>
    <x-slot name="heading">Collection Type</x-slot>
                ...
                <div class="card">
                    <form class="theme-form" name="addForm" id="addForm" enctype="multipart/form-data">
                        ...
                        <div class="card-header">
                            <h4 class="card-title">Add Collection Type</h4>
                        </div>
                        <div class="card-body">
                            <div class="mb-3 row">
                                <div class="col-md-4">
        ...
        <div class="row" id="editContainer" style="display:none;">
            <div class="col">
                <form class="form-horizontal form-bordered" method="post" id="editForm">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Edit Collection Type</h4>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="buttons-datatables" class="table table-bordered align-middle" style="width:100%">
                                ...
                                <tbody>
                                    {{-- @php $count = 1; @endphp --}}
                                    @foreach ($CollectionType as $collection)
                                        <tr>
                                            {{-- <td>{{ $count++ }}</td> --}}
                                            ...
                                            <td>
                                                <button class="edit-element btn text-secondary px-2 py-1" title="Edit CollectionType" data-id="{{ $collection->id }}"><i data-feather="edit"></i></button>
                                                <button class="btn text-danger rem-element px-2 py-1" title="Delete CollectionType" data-id="{{ $collection->id }}"><i data-feather="trash-2"></i> </button>
                                            </td>
                                        </tr>
                                    @endforeach
                            </table>
...
<script>
    $("#addForm").submit(function(e) {
        ...
        $.ajax({
            url: '{{ route('collection-type.store') }}',
            type: 'POST',
            data: formdata,
            contentType: false,
            processData: false,
            success: function(data)
            {
                ...
                if (!data.error2)
                    swal("Successful!", data.success, "success")
                        .then((action) => {
                            window.location.href = '{{ route('collection-type.index') }}';
                        });
                else
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('collection-type.edit', ":model_id") }}";
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (!data.error)
                {
                    $("#editForm input[id='edit_model_id']").val(data.CollectionType.id);
                    $("#editForm input[name='Description']").val(data.CollectionType.Description);
                    $("#editForm input[name='Description_regional']").val(data.CollectionType.Description_regional);
                    $("#editForm input[name='value']").val(data.CollectionType.value);
                    $("#editForm input[name='other_value']").val(data.CollectionType.other_value);
                }
                else
                {
                    alert(data.error);
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('collection-type.update', ":model_id") }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
...
<script>
    $("#buttons-datatables").on("click", ".rem-element", function(e) {
        ...
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('collection-type.destroy', ":model_id") }}";
