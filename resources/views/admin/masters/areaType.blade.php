            ...
            success: function(data)
            {
                ...
                if (!data.error2)
                    swal("Successful!", data.success, "success")
                        .then((action) => {
                            window.location.href = '{{ route('area-type.index') }}';
                        });
                else
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('area-type.edit', ":model_id") }}";
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (!data.error)
                {
                    $("#editForm input[id='edit_model_id']").val(data.AreaType.id);
                    $("#editForm input[name='Description']").val(data.AreaType.Description);
                    $("#editForm input[name='Description_regional']").val(data.AreaType.Description_regional);
                    $("#editForm input[name='value']").val(data.AreaType.value);
                    $("#editForm input[name='other_value']").val(data.AreaType.other_value);
                }
                else
                {
                    alert(data.error);
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('area-type.update', ":model_id") }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
...
<script>
    $("#buttons-datatables").on("click", ".rem-element", function(e) {
        ...
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('area-type.destroy', ":model_id") }}";
